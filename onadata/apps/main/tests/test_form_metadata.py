...
class TestFormMetadata(TestBase):
    ...
    def _count_datatype(self, data_type):
        return len(MetaData.type_for_form(self.xform, data_type))
    ...
    def test_adds_supporting_doc_on_submit(self):
        count = self._count_datatype('supporting_doc')
        self._add_metadata()
        self.assertEquals(count + 1, self._count_datatype('supporting_doc'))

    def test_adds_supporting_media_on_submit(self):
        count = self._count_datatype('media')
        self._add_metadata(data_type='media')
        self.assertEquals(count + 1, self._count_datatype('media'))

    def test_adds_mapbox_layer_on_submit(self):
        count = self._count_datatype('mapbox_layer')
        self.post_data = {}
        self.post_data['map_name'] = 'test_mapbox_layer'
        self.post_data['link'] = 'http://0.0.0.0:8080'
        self.client.post(self.edit_url, self.post_data)
        self.assertEquals(count + 1, self._count_datatype('mapbox_layer'))
    ...
    def test_delete_supporting_doc(self):
        count = self._count_datatype('supporting_doc')
        self._add_metadata()
        self.assertEqual(count + 1, self._count_datatype('supporting_doc'))
        ...
        self.assertEqual(count, self._count_datatype('supporting_doc'))
        self.assertEqual(response.status_code, 302)

    def test_delete_supporting_media(self):
        count = self._count_datatype('media')
        self._add_metadata(data_type='media')
        self.assertEqual(count + 1, self._count_datatype('media'))
        response = self.client.get(self.doc_url + '?del=true')
        self.assertEqual(count, self._count_datatype('media'))
        self.assertEqual(response.status_code, 302)
        self._add_metadata(data_type='media')
    ...
    def _add_mapbox_layer(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(count + 1, self._count_datatype('mapbox_layer'))
        return count

    def test_delete_mapbox_layer(self):
        count = self._add_mapbox_layer()
        ...
        self.assertEqual(count, self._count_datatype('mapbox_layer'))
        self.assertEqual(response.status_code, 302)
